...
echo Boot parameters: root=LABEL=ROOT ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic console=tty0 console=ttyS0,115200n8 selinux=0 enforcing=0 pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off
\\vmlinuz root=LABEL=ROOT ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic console=tty0 console=ttyS0,115200n8 selinux=0 enforcing=0 pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off initrd=\\initrd
"""
...
menuentry 'Nutanix AHV' --unrestricted --id nutanix {{
 ...
 linux /vmlinuz root=LABEL=ROOT ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic console=tty0 console=ttyS0,115200n8 selinux=0 enforcing=0 quiet pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off
   ...
   for INITRD_PATH in /boot/initramfs-{kernel_version}.img /initrd /boot/initramfs-*.img; do
     if [ -f "$INITRD_PATH" ]; then
       ...
       for ROOT_SPEC in "root=/dev/{boot_disk}p2" "root=LABEL=ROOT" "root=UUID=$(blkid -s UUID -o value /dev/{boot_disk}p2 2>/dev/null || echo 'none')"; do
         ...
         kexec -l $KERNEL_PATH --initrd=$INITRD_PATH --command-line="$ROOT_SPEC ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic console=tty0 console=ttyS0,115200n8 selinux=0 enforcing=0 pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off" 2>/dev/null
...
GRUB_CMDLINE_LINUX="root=/dev/{boot_disk}p2 ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic ip=dhcp rd.neednet=1 console=tty0 console=ttyS0,115200n8 selinux=0 enforcing=0 debug pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off"
...
menuentry "Nutanix AHV (rescue mode)" {{
   search --no-floppy --label ROOT --set=root
   linux /vmlinuz root=LABEL=ROOT ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic console=tty0 console=ttyS0,115200n8 selinux=0 enforcing=0 single pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off
   initrd /initrd
}}
"""
               with open('/mnt/stage/boot/grub2/grub.cfg', 'w') as f:
...
echo Loading kernel: vmlinuz
echo Loading initrd: initrd
echo Boot parameters: root=/dev/{boot_disk}p2 ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic ip=dhcp rd.neednet=1 console=tty0 console=ttyS0,115200n8 debug pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off
\\vmlinuz root=/dev/{boot_disk}p2 ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic ip=dhcp rd.neednet=1 console=tty0 console=ttyS0,115200n8 debug pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off initrd=\\initrd
 ...
 if [ -f "$KERNEL_PATH" ]; then
   ...
   for INITRD_PATH in /boot/initramfs-{kernel_version}.img /initrd /boot/initramfs-*.img; do
     if [ -f "$INITRD_PATH" ]; then
       ...
       for ROOT_SPEC in "root=/dev/{boot_disk}p2" "root=LABEL=ROOT" "root=UUID=$(blkid -s UUID -o value /dev/{boot_disk}p2 2>/dev/null || echo 'none')"; do
         echo "Attempting boot with $ROOT_SPEC"
         echo "kexec -l $KERNEL_PATH --initrd=$INITRD_PATH --command-line=\"$ROOT_SPEC ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic ip=dhcp rd.neednet=1 console=tty0 console=ttyS0,115200n8 debug selinux=0 enforcing=0 pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off\""
         
         kexec -l $KERNEL_PATH --initrd=$INITRD_PATH --command-line="$ROOT_SPEC ro crashkernel=auto net.ifnames=0 nvme.io_timeout=4294967295 modprobe.blacklist=mlx4_core,mlx4_en,mlx4_ib,ionic ip=dhcp rd.neednet=1 console=tty0 console=ttyS0,115200n8 debug selinux=0 enforcing=0 pci=realloc=on,nocrs,noaer,assign-busses,hpiosize=0x4000,hpmemsize=0x10000000,nobar pcie_aspm=off iommu=pt acpi=off" 2>/dev/null
